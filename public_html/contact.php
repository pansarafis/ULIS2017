<!DOCTYPE html>
<html lang="en">
<head>
    <?php include 'header.php'?>
</head>
<body>

<div class="container">
    
    
    <?php include 'title.php'?>
    <?php include 'menu.php'?>
 

    <div class="main row">

        <div class="chipImg">
            <img src="images/chipgrey.png" />
        </div>

        <div>
            
            <div class="col-sm-5">
                <h3>Conference Chair</h3>
                <p>Androula G. Nassiopoulou</p>
                
                <h3>Contact Details</h3>
                <p class="caption">
                    <icon class="glyphicon glyphicon-home"></icon> NCSR "Demokritos", Ag. Parasvevi, 15310, Athens - Greece <br>
                    <icon class="glyphicon glyphicon-phone-alt"></icon> Tel: +30 210 650 3411<br>
                    <icon class="glyphicon glyphicon-print"></icon> Fax: +30 210 650 3411<br>
                    <icon class="glyphicon glyphicon-envelope"></icon> E-mail: <a href="mailto:a.nasiopoulou@inn.demokritos.gr">a.nasiopoulou@inn.demokritos.gr</a><br>
                    <icon class="glyphicon glyphicon-globe"></icon> Website: <a href="http://nano4nps.imel.demokritos.gr">http://nano4nps.imel.demokritos.gr</a>
                </p>  
            </div>
                      
            <div class="embed-responsive embed-responsive-16by9 col-sm-7">
                <iframe class="embed-responsive-item" src="https://www.google.com/maps/d/embed?mid=1khiXfVFFC3Y3mFi0N_NwynT4IiY" width="640" height="480"></iframe>
            </div>

        </div>
    </div>
    
    <div class="footer row">
        <p>Copyright EuroSOI-ULIS 2017</p>
    </div>
</div>

</body>
</html>